...
export default function App() {
  ...
  const [showShareModal, setShowSahreModal] = useState(false)
  ...
  const urlShare = window.location.href + `?file=` + selectedFile?.link
  return (
    <>
      {showChartModal && (
      ...
      {showShareModal && (
        <div style={styles.modal}>
          <div style={styles.modalContent}>
            <div style={styles.modalHeader}>
              <p style={{ fontWeight: "bold" }}>Files Breakdown</p>
              <button style={styles.closeButton} onClick={() => setShowSahreModal(false)}>close</button>
            </div>
            <div style={styles.modalBodyShare}>
              <div>Your Link</div><br />
              <div>
                <a href={urlShare} target='_blank' rel='noreferrer'>{urlShare}</a>
              </div>
              <button
                style={{ marginTop: 10 }}
                onClick={() => {
                  navigator.clipboard.writeText(urlShare).then(() => window.alert("copy success"))
                }}>Copy URL</button>
            </div>
          </div>
        </div>
        ...
        <div style={styles.container}>
          ...
          <div style={styles.controlTools}>
            ...
            <button style={styles.controlButton}
              onClick={() => {
                if (selectedFile) {
                  setMyFiles(mf => mf.filter(f => f.id != selectedFile.id))
                  setSelectedFile(null)
                }
              }}
            >Delete</button>
              ...
              {myFiles
                .filter(file => {
                  if (filter.type.length > 0) {
                    return filter.type.includes(file.type)
                  }
                  return true
                })
                .filter(file => {
                  return file.name.toLowerCase().includes(filter.search.toLowerCase())
                }).map((file) => {
